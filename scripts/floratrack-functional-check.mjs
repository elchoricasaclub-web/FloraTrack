...
const staticOnly = args.has("--static") || process.env.FLORATRACK_STATIC_ONLY === "1";
const sourceOnly = staticOnly || process.env.FLORATRACK_SOURCE_ONLY === "1" || process.argv.includes("--source-only");
...
async function main() {
  ...
  const serverReady = sourceOnly ? false : await waitForServer(errors);
  ...
  console.log("FLORATRACK FUNCTIONAL CHECK - OK");
  console.log(`Mode: ${sourceOnly ? "static" : "http"}`);
