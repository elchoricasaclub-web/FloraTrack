...
export default function BatchDashboard() {
  ...
  // QR modal states
  const [activeQrData, setActiveQrData] = useState<{ name: string; barcode: string; url: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Parse URL search parameters on compile/mount to auto-select/filter scanned batches
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const qrSearch = params.get('search') || params.get('lote');
      if (qrSearch) {
        setSearchTerm(qrSearch);
      }
    }
  }, []);
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-slate-900">FloraTrack Premium AgroTech</h1>
              <p className="text-xs text-slate-500 font-medium tracking-wide">PANEL DE TRAZABILIDAD SAAS GACP & GMP</p>
            </div>
          </div>
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      ...
                      <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between">
                        <div>
                          <div className="text-[10px] leading-relaxed text-slate-400 font-black uppercase tracking-wider block mb-0.5">Rastreabilidad (GTIN)</div>
                          <div className="text-xs font-bold text-slate-700 truncate">{batch.barcode}</div>
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            const qrUrl = typeof window !== 'undefined'
                              ? `${window.location.origin}${window.location.pathname}?search=${encodeURIComponent(batch.barcode)}`
                              : `https://floratrack.com?search=${batch.barcode}`;
                            setActiveQrData({
                              name: batch.name,
                          ...
                          className="mt-2 py-1 px-2.5 bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-[9px] uppercase tracking-wider rounded-lg transition-all flex items-center justify-center gap-1.5 self-start shadow-sm active:scale-95"
                          title="Generar e Imprimir Código QR para este Lote"
                        >
                          <svg className="w-3 h-3 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v1m0 11v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707" />
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 text-slate-900 max-w-sm w-full relative shadow-2xl overflow-hidden animate-fade-in text-center">
            <button
              ...
              className="absolute top-4 right-4 w-7 h-7 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 transition text-slate-500 font-bold"
            >
              ×
            </button>
            <div className="space-y-4">
              <span className="text-[10px] text-emerald-600 font-extrabold uppercase tracking-widest block">
                FLORATRACK LOTES REGULATORY QR
              </span>
              <div>
                <h3 className="text-sm font-black text-slate-900 leading-none">{activeQrData.name}</h3>
                <p className="text-[10px] text-slate-500 font-mono mt-1 pt-1 bg-slate-100 rounded">Rastreabilidad: {activeQrData.barcode}</p>
              </div>
              
              <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl inline-block max-w-[220px] aspect-square mx-auto shadow-inner">
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(activeQrData.url)}&color=0f172a&bgcolor=ffffff&margin=1`}
                  alt="QR Code"
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 text-[10px] text-slate-500 font-semibold leading-relaxed text-left">
                Escanea este QR con un dispositivo móvil para recuperar al instante el historial de auditoría y conformidad GACP/GMP.
              </div>

              <div className="grid grid-cols-2 gap-2">
                <a
                  href={`https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${encodeURIComponent(activeQrData.url)}&color=0f172a&bgcolor=ffffff&margin=1`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl transition text-center shadow-md shadow-emerald-500/10"
                >
                  Ver Alta Res
                </a>
                <button
                  ...
                  onClick={() => {
                    ...
                    link.href = `https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${encodeURIComponent(activeQrData.url)}&color=0f172a&bgcolor=ffffff&margin=1`;
                    link.download = `QR_${activeQrData.barcode}.png`;
                    link.target = '_blank';
                    link.click();
                  }}
                  className="py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs rounded-xl transition text-center border border-slate-200"
                >
                  Exportar PNG
                </button>
              </div>
