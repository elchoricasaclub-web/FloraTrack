"use client";

import { useEffect, useMemo, useState } from "react";

type WasteRecord = {
  id: string;
  codigoResiduo: string;
  fecha: string;
  hora: string;
  empresa: string;
  sede: string;
  areaOrigen: string;
  ubicacion: string;
  tipoResiduo: string;
  descripcionResiduo: string;
  codigoLoteRelacionado: string;
  cantidad: string;
  unidad: string;
  clasificacionRiesgo: string;
  contenedor: string;
  responsableGeneracion: string;
  responsableEntrega: string;
  gestorAutorizado: string;
  requiereTratamiento: string;
  tratamientoAplicado: string;
  metodoDisposicion: string;
  fechaDisposicion: string;
  certificadoDisposicion: string;
  estadoGestion: string;
  decisionQA: string;
  desviacionAsociada: string;
  accionCorrectiva: string;
  evidencia: string;
  observaciones: string;
  creadoEn: string;
  actualizadoEn: string;
};

type FieldConfig = {
  key: keyof WasteRecord;
  label: string;
  type?: string;
  placeholder?: string;
  kind?: "input" | "select" | "textarea";
  options?: string[];
};

type CloudTone = "success" | "warning";

type CloudState = {
  message: string;
  errors: string[];
  tone: CloudTone;
};

const STORAGE_KEY = "floratrack_residuos_disposicion_v1";

const emptyForm: WasteRecord = {
  id: "",
  codigoResiduo: "",
  fecha: "",
  hora: "",
  empresa: "",
  sede: "",
  areaOrigen: "",
  ubicacion: "",
  tipoResiduo: "",
  descripcionResiduo: "",
  codigoLoteRelacionado: "",
  cantidad: "",
  unidad: "",
  clasificacionRiesgo: "",
  contenedor: "",
  responsableGeneracion: "",
  responsableEntrega: "",
  gestorAutorizado: "",
  requiereTratamiento: "",
  tratamientoAplicado: "",
  metodoDisposicion: "",
  fechaDisposicion: "",
  certificadoDisposicion: "",
  estadoGestion: "",
  decisionQA: "",
  desviacionAsociada: "",
  accionCorrectiva: "",
  evidencia: "",
  observaciones: "",
  creadoEn: "",
  actualizadoEn: "",
};

const requiredFields: Array<keyof WasteRecord> = [
  "codigoResiduo",
  "fecha",
  "hora",
  "empresa",
  "sede",
  "areaOrigen",
  "ubicacion",
  "tipoResiduo",
  "descripcionResiduo",
  "cantidad",
  "unidad",
  "clasificacionRiesgo",
  "contenedor",
  "responsableGeneracion",
  "responsableEntrega",
  "requiereTratamiento",
  "metodoDisposicion",
  "estadoGestion",
  "decisionQA",
];

const fieldLabels: Record<keyof WasteRecord, string> = {
  id: "ID",
  codigoResiduo: "Código de residuo",
  fecha: "Fecha",
  hora: "Hora",
  empresa: "Empresa",
  sede: "Sede / predio",
  areaOrigen: "Área de origen",
  ubicacion: "Ubicación",
  tipoResiduo: "Tipo de residuo",
  descripcionResiduo: "Descripción del residuo",
  codigoLoteRelacionado: "Lote relacionado",
  cantidad: "Cantidad",
  unidad: "Unidad",
  clasificacionRiesgo: "Clasificación de riesgo",
  contenedor: "Contenedor",
  responsableGeneracion: "Responsable de generación",
  responsableEntrega: "Responsable de entrega",
  gestorAutorizado: "Gestor autorizado",
  requiereTratamiento: "Requiere tratamiento",
  tratamientoAplicado: "Tratamiento aplicado",
  metodoDisposicion: "Método de disposición",
  fechaDisposicion: "Fecha de disposición",
  certificadoDisposicion: "Certificado / manifiesto",
  estadoGestion: "Estado de gestión",
  decisionQA: "Decisión QA",
  desviacionAsociada: "Desviación asociada",
  accionCorrectiva: "Acción correctiva / CAPA",
  evidencia: "Evidencia",
  observaciones: "Observaciones",
  creadoEn: "Creado en",
  actualizadoEn: "Actualizado en",
};

const fields: FieldConfig[] = [
  { key: "codigoResiduo", label: "Código de residuo *", placeholder: "RES-2026-001" },
  { key: "fecha", label: "Fecha *", type: "date" },
  { key: "hora", label: "Hora *", type: "time" },
  { key: "empresa", label: "Empresa *", placeholder: "Growlifecol S.A.S." },
  { key: "sede", label: "Sede / predio *", placeholder: "Sede principal" },
  {
    key: "areaOrigen",
    label: "Área de origen *",
    kind: "select",
    options: [
      "Cultivo",
      "Propagación",
      "Cosecha",
      "Secado",
      "Postcosecha",
      "Micropropagación",
      "Extracción BHO",
      "Bubble Hash",
      "Live Rosin",
      "QA",
      "QC",
      "Inventario",
      "Recepción",
      "Almacén",
      "Limpieza y saneamiento",
      "Mantenimiento",
      "Residuos",
      "Áreas comunes",
    ],
  },
  { key: "ubicacion", label: "Ubicación *", placeholder: "Cuarto de residuos, invernadero, laboratorio, sala de extracción..." },
  {
    key: "tipoResiduo",
    label: "Tipo de residuo *",
    kind: "select",
    options: [
      "Vegetal no aprovechable",
      "Biomasa rechazada",
      "Sustrato usado",
      "Residuos orgánicos",
      "Material de empaque",
      "Plástico / cartón",
      "Vidrio",
      "Peligroso",
      "Químico",
      "Biológico",
      "Solventes / extracción",
      "Filtro / consumible contaminado",
      "Equipo o pieza retirada",
      "Otro",
    ],
  },
  {
    key: "descripcionResiduo",
    label: "Descripción del residuo *",
    kind: "textarea",
    placeholder: "Describe origen, condición, posible contaminación, relación con lote, proceso o desviación.",
  },
  { key: "codigoLoteRelacionado", label: "Lote relacionado", placeholder: "LOT-2026-001 / cultivo / extracción / muestra QC" },
  { key: "cantidad", label: "Cantidad *", type: "number", placeholder: "10" },
  {
    key: "unidad",
    label: "Unidad *",
    kind: "select",
    options: ["kg", "g", "L", "mL", "unidades", "bolsas", "contenedores", "cajas"],
  },
  {
    key: "clasificacionRiesgo",
    label: "Clasificación de riesgo *",
    kind: "select",
    options: ["Bajo", "Medio", "Alto", "Crítico"],
  },
  { key: "contenedor", label: "Contenedor *", placeholder: "Bolsa roja, contenedor rotulado, caneca verde, tambor certificado..." },
  { key: "responsableGeneracion", label: "Responsable de generación *", placeholder: "Nombre del responsable del área" },
  { key: "responsableEntrega", label: "Responsable de entrega *", placeholder: "Nombre de quien entrega o transfiere el residuo" },
  { key: "gestorAutorizado", label: "Gestor autorizado", placeholder: "Empresa gestora / proveedor autorizado" },
  {
    key: "requiereTratamiento",
    label: "Requiere tratamiento *",
    kind: "select",
    options: ["Sí", "No"],
  },
  {
    key: "tratamientoAplicado",
    label: "Tratamiento aplicado",
    kind: "textarea",
    placeholder: "Inactivación, trituración, neutralización, segregación, cuarentena, desnaturalización, etc.",
  },
  {
    key: "metodoDisposicion",
    label: "Método de disposición *",
    kind: "select",
    options: [
      "Compostaje autorizado",
      "Aprovechamiento interno permitido",
      "Entrega a gestor",
      "Incineración",
      "Relleno autorizado",
      "Reciclaje",
      "Neutralización",
      "Desnaturalización",
      "Cuarentena temporal",
      "Pendiente disposición",
      "Otro",
    ],
  },
  { key: "fechaDisposicion", label: "Fecha de disposición", type: "date" },
  { key: "certificadoDisposicion", label: "Certificado / manifiesto", placeholder: "CERT-RES-001 / manifiesto / acta / enlace documental" },
  {
    key: "estadoGestion",
    label: "Estado de gestión *",
    kind: "select",
    options: [
      "Generado",
      "Segregado",
      "En cuarentena",
      "Pendiente entrega",
      "Entregado a gestor",
      "Dispuesto",
      "Rechazado",
      "Requiere investigación",
      "Requiere CAPA",
    ],
  },
  {
    key: "decisionQA",
    label: "Decisión QA *",
    kind: "select",
    options: [
      "Pendiente QA",
      "Liberado QA",
      "Retenido QA",
      "Rechazado QA",
      "Requiere desviación",
      "Requiere CAPA",
      "No aplica",
    ],
  },
  { key: "desviacionAsociada", label: "Desviación asociada", placeholder: "DEV-2026-001 / CAPA-001" },
  {
    key: "accionCorrectiva",
    label: "Acción correctiva / CAPA",
    kind: "textarea",
    placeholder: "Obligatoria si hay rechazo, riesgo alto/crítico, pérdida de trazabilidad o disposición no conforme.",
  },
  { key: "evidencia", label: "Evidencia / soporte", placeholder: "Foto, pesaje, acta, manifiesto, certificado, QR, firma, registro de entrega..." },
  {
    key: "observaciones",
    label: "Observaciones",
    kind: "textarea",
    placeholder: "Notas QA, condiciones, restricciones, impacto sobre lotes, seguimiento.",
  },
];

function clean(value: unknown): string {
  return String(value ?? "").replace(/\s+/g, " ").trim();
}

function isInvalid(value: unknown): boolean {
  return ["", "seleccione", "seleccionar", "sin definir", "n/a", "na", "ninguno", "null", "undefined"].includes(
    clean(value).toLowerCase()
  );
}

function safeId(): string {
  if (typeof crypto !== "undefined" && "randomUUID" in crypto) return crypto.randomUUID();
  return `RES-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}

function nowIso(): string {
  return new Date().toISOString();
}

function loadRecords(): WasteRecord[] {
  if (typeof window === "undefined") return [];

  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? parsed.filter(Boolean) : [];
  } catch {
    return [];
  }
}

function saveRecords(records: WasteRecord[]) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify(records));
}

export default function ResiduosPage() {
  const [records, setRecords] = useState<WasteRecord[]>([]);
  const [form, setForm] = useState<WasteRecord>(emptyForm);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [cloud, setCloud] = useState<CloudState | null>(null);
  const [validationErrors, setValidationErrors] = useState<string[]>([]);
  const [submitAttempted, setSubmitAttempted] = useState(false);
  const [search, setSearch] = useState("");
  const [filterEstado, setFilterEstado] = useState("Todos");

  useEffect(() => {
    setRecords(loadRecords());
  }, []);

  function showCloud(message: string, errors: string[] = [], tone: CloudTone = "warning") {
    setCloud({ message, errors, tone });
    setValidationErrors(errors);

    window.setTimeout(() => {
      setCloud(null);
      if (errors.length === 0) setValidationErrors([]);
    }, errors.length > 0 ? 12000 : 6000);
  }

  function updateField(field: keyof WasteRecord, value: string) {
    setForm((current) => ({ ...current, [field]: value }));
  }

  function needsCapa(): boolean {
    return (
      ["Alto", "Crítico"].includes(form.clasificacionRiesgo) ||
      ["Rechazado", "Requiere investigación", "Requiere CAPA"].includes(form.estadoGestion) ||
      ["Retenido QA", "Rechazado QA", "Requiere desviación", "Requiere CAPA"].includes(form.decisionQA)
    );
  }

  function validateForm(): string[] {
    const errors = requiredFields
      .filter((field) => isInvalid(form[field]))
      .map((field) => fieldLabels[field]);

    const amount = Number(form.cantidad);

    if (!Number.isFinite(amount) || amount <= 0) {
      errors.push("La cantidad debe ser mayor a cero");
    }

    const hazardous = ["Peligroso", "Químico", "Biológico", "Solventes / extracción", "Filtro / consumible contaminado"].includes(form.tipoResiduo);

    if (hazardous && isInvalid(form.gestorAutorizado)) {
      errors.push("El gestor autorizado es obligatorio para residuos peligrosos, químicos, biológicos o contaminados");
    }

    if (hazardous && isInvalid(form.certificadoDisposicion)) {
      errors.push("El certificado o manifiesto es obligatorio para residuos peligrosos, químicos, biológicos o contaminados");
    }

    if (form.requiereTratamiento === "Sí" && isInvalid(form.tratamientoAplicado)) {
      errors.push("El tratamiento aplicado es obligatorio cuando el residuo requiere tratamiento");
    }

    if (["Entregado a gestor", "Dispuesto"].includes(form.estadoGestion) && isInvalid(form.fechaDisposicion)) {
      errors.push("La fecha de disposición es obligatoria cuando el residuo fue entregado o dispuesto");
    }

    if (["Liberado QA", "Retenido QA", "Rechazado QA"].includes(form.decisionQA) && isInvalid(form.evidencia)) {
      errors.push("La evidencia es obligatoria cuando QA libera, retiene o rechaza");
    }

    if (needsCapa() && isInvalid(form.desviacionAsociada)) {
      errors.push("La desviación asociada es obligatoria cuando existe riesgo alto/crítico o condición no conforme");
    }

    if (needsCapa() && isInvalid(form.accionCorrectiva)) {
      errors.push("La acción correctiva / CAPA es obligatoria cuando existe riesgo alto/crítico o condición no conforme");
    }

    return Array.from(new Set(errors));
  }

  function fieldHasError(field: keyof WasteRecord): boolean {
    if (!submitAttempted) return false;

    const hazardous = ["Peligroso", "Químico", "Biológico", "Solventes / extracción", "Filtro / consumible contaminado"].includes(form.tipoResiduo);

    if (requiredFields.includes(field) && isInvalid(form[field])) return true;

    if (field === "cantidad") {
      const amount = Number(form.cantidad);
      return !Number.isFinite(amount) || amount <= 0;
    }

    if (field === "gestorAutorizado" && hazardous && isInvalid(form.gestorAutorizado)) return true;
    if (field === "certificadoDisposicion" && hazardous && isInvalid(form.certificadoDisposicion)) return true;
    if (field === "tratamientoAplicado" && form.requiereTratamiento === "Sí" && isInvalid(form.tratamientoAplicado)) return true;
    if (field === "fechaDisposicion" && ["Entregado a gestor", "Dispuesto"].includes(form.estadoGestion) && isInvalid(form.fechaDisposicion)) return true;
    if (field === "evidencia" && ["Liberado QA", "Retenido QA", "Rechazado QA"].includes(form.decisionQA) && isInvalid(form.evidencia)) return true;
    if (field === "desviacionAsociada" && needsCapa() && isInvalid(form.desviacionAsociada)) return true;
    if (field === "accionCorrectiva" && needsCapa() && isInvalid(form.accionCorrectiva)) return true;

    return false;
  }

  function resetForm() {
    setForm(emptyForm);
    setEditingId(null);
    setSubmitAttempted(false);
    setValidationErrors([]);
    setCloud(null);
  }

  function handleSave() {
    setSubmitAttempted(true);

    const errors = validateForm();

    if (errors.length > 0) {
      showCloud("No se guardó el residuo. Completa la información obligatoria.", errors, "warning");
      return;
    }

    const timestamp = nowIso();

    const payload: WasteRecord = {
      ...form,
      codigoResiduo: clean(form.codigoResiduo),
      fecha: clean(form.fecha),
      hora: clean(form.hora),
      empresa: clean(form.empresa),
      sede: clean(form.sede),
      areaOrigen: clean(form.areaOrigen),
      ubicacion: clean(form.ubicacion),
      tipoResiduo: clean(form.tipoResiduo),
      descripcionResiduo: clean(form.descripcionResiduo),
      codigoLoteRelacionado: clean(form.codigoLoteRelacionado),
      cantidad: clean(form.cantidad),
      unidad: clean(form.unidad),
      clasificacionRiesgo: clean(form.clasificacionRiesgo),
      contenedor: clean(form.contenedor),
      responsableGeneracion: clean(form.responsableGeneracion),
      responsableEntrega: clean(form.responsableEntrega),
      gestorAutorizado: clean(form.gestorAutorizado),
      requiereTratamiento: clean(form.requiereTratamiento),
      tratamientoAplicado: clean(form.tratamientoAplicado),
      metodoDisposicion: clean(form.metodoDisposicion),
      fechaDisposicion: clean(form.fechaDisposicion),
      certificadoDisposicion: clean(form.certificadoDisposicion),
      estadoGestion: clean(form.estadoGestion),
      decisionQA: clean(form.decisionQA),
      desviacionAsociada: clean(form.desviacionAsociada),
      accionCorrectiva: clean(form.accionCorrectiva),
      evidencia: clean(form.evidencia),
      observaciones: clean(form.observaciones),
      id: editingId ?? safeId(),
      creadoEn: editingId ? form.creadoEn : timestamp,
      actualizadoEn: timestamp,
    };

    const nextRecords = editingId
      ? records.map((record) => (record.id === editingId ? payload : record))
      : [payload, ...records];

    setRecords(nextRecords);
    saveRecords(nextRecords);
    resetForm();

    showCloud(
      editingId ? "Residuo actualizado correctamente." : "Residuo registrado correctamente con control GMP/GACP.",
      [],
      "success"
    );
  }

  function handleEdit(record: WasteRecord) {
    setForm(record);
    setEditingId(record.id);
    setSubmitAttempted(false);
    showCloud("Registro cargado para edición. Verifica antes de actualizar.", [], "success");
  }

  function handleDelete(id: string) {
    const confirmed = window.confirm("¿Confirmas eliminar este registro? En ambiente GMP real debería manejarse como anulación auditada.");
    if (!confirmed) return;

    const nextRecords = records.filter((record) => record.id !== id);
    setRecords(nextRecords);
    saveRecords(nextRecords);
    showCloud("Registro eliminado del almacenamiento local.", [], "success");
  }

  function exportJson() {
    if (records.length === 0) {
      showCloud("No hay residuos para exportar.", [], "warning");
      return;
    }

    const blob = new Blob([JSON.stringify(records, null, 2)], { type: "application/json;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `floratrack-residuos-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
    URL.revokeObjectURL(url);

    showCloud("Archivo JSON de residuos exportado correctamente.", [], "success");
  }

  const filteredRecords = useMemo(() => {
    const term = clean(search).toLowerCase();

    return records.filter((record) => {
      const matchesSearch =
        !term ||
        [
          record.codigoResiduo,
          record.empresa,
          record.sede,
          record.areaOrigen,
          record.tipoResiduo,
          record.codigoLoteRelacionado,
          record.clasificacionRiesgo,
          record.estadoGestion,
          record.decisionQA,
        ]
          .join(" ")
          .toLowerCase()
          .includes(term);

      const matchesEstado = filterEstado === "Todos" || record.estadoGestion === filterEstado;

      return matchesSearch && matchesEstado;
    });
  }, [records, search, filterEstado]);

  const dashboard = useMemo(() => {
    return {
      total: records.length,
      dispuesto: records.filter((record) => record.estadoGestion === "Dispuesto").length,
      cuarentena: records.filter((record) => record.estadoGestion === "En cuarentena").length,
      altoRiesgo: records.filter((record) => ["Alto", "Crítico"].includes(record.clasificacionRiesgo)).length,
      pendienteQA: records.filter((record) => record.decisionQA === "Pendiente QA").length,
    };
  }, [records]);

  return (
    <main className="min-h-screen bg-slate-950 px-6 py-8 text-slate-100">
      {cloud && (
        <CloudNotice
          message={cloud.message}
          errors={cloud.errors}
          tone={cloud.tone}
          onClose={() => {
            setCloud(null);
            if (cloud.errors.length === 0) setValidationErrors([]);
          }}
        />
      )}

      <section className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-3xl border border-rose-400/20 bg-slate-900/80 p-6 shadow-2xl shadow-rose-950/30">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.35em] text-rose-300">
                FloraTrack Enterprise Compliance Platform
              </p>

              <h1 className="mt-3 text-3xl font-black text-white md:text-5xl">
                Residuos y disposición GMP/GACP
              </h1>

              <p className="mt-3 max-w-5xl text-sm leading-6 text-slate-300 md:text-base">
                Control de residuos por área, lote, clasificación de riesgo, cantidad,
                contenedor, tratamiento, gestor autorizado, disposición final, certificado,
                evidencia, QA, desviaciones y CAPA.
              </p>
            </div>

            <div className="rounded-2xl border border-rose-400/20 bg-rose-500/10 px-5 py-4 text-sm text-rose-100">
              <p className="font-bold">Gestión ambiental GMP activa</p>
              <p className="mt-1 text-rose-200">Residuos · Gestor · Disposición · QA · CAPA</p>
            </div>
          </div>
        </header>

        {submitAttempted && validationErrors.length > 0 && (
          <section className="rounded-3xl border border-red-400/40 bg-red-500/10 p-5 text-red-100">
            <h2 className="text-lg font-black">No se puede guardar todavía</h2>
            <p className="mt-1 text-sm text-red-100/90">
              Completa la información obligatoria antes de crear el registro electrónico.
            </p>

            <div className="mt-3 grid gap-2 md:grid-cols-2">
              {validationErrors.map((error) => (
                <div key={error} className="rounded-xl border border-red-400/30 bg-red-950/40 px-4 py-2 text-sm">
                  {error}
                </div>
              ))}
            </div>
          </section>
        )}

        <section className="grid gap-4 md:grid-cols-5">
          <Metric title="Residuos" value={dashboard.total} />
          <Metric title="Dispuestos" value={dashboard.dispuesto} tone="emerald" />
          <Metric title="Cuarentena" value={dashboard.cuarentena} tone="amber" />
          <Metric title="Alto riesgo" value={dashboard.altoRiesgo} tone="red" />
          <Metric title="Pendiente QA" value={dashboard.pendienteQA} tone="sky" />
        </section>

        <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <form
            noValidate
            className="rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-xl"
            onSubmit={(event) => {
              event.preventDefault();
              handleSave();
            }}
          >
            <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 className="text-2xl font-black text-white">{editingId ? "Editar residuo" : "Nuevo residuo"}</h2>
                <p className="mt-1 text-sm text-slate-400">Ningún residuo puede guardarse vacío o incompleto.</p>
              </div>

              {editingId && (
                <button type="button" onClick={resetForm} className="rounded-xl border border-slate-600 px-4 py-2 text-sm font-bold text-slate-200 hover:bg-slate-800">
                  Cancelar edición
                </button>
              )}
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              {fields.map((field) => {
                const hasError = fieldHasError(field.key);
                const controlClass = hasError
                  ? "border-red-400 bg-red-950/30 ring-4 ring-red-400/20"
                  : "border-slate-700 bg-slate-950 focus:border-rose-400 focus:ring-4 focus:ring-rose-400/40";

                if (field.kind === "textarea") {
                  return (
                    <label key={field.key} className="md:col-span-2">
                      <span className="mb-2 block text-sm font-bold text-slate-200">{field.label}</span>

                      <textarea
                        value={form[field.key]}
                        onChange={(event) => updateField(field.key, event.target.value)}
                        rows={4}
                        placeholder={field.placeholder}
                        className={`w-full rounded-2xl border px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 ${controlClass}`}
                      />

                      {hasError && <p className="mt-1 text-xs font-bold text-red-300">Campo obligatorio o condición ambiental/GMP requerida.</p>}
                    </label>
                  );
                }

                if (field.kind === "select") {
                  return (
                    <label key={field.key}>
                      <span className="mb-2 block text-sm font-bold text-slate-200">{field.label}</span>

                      <select
                        value={form[field.key]}
                        onChange={(event) => updateField(field.key, event.target.value)}
                        className={`w-full rounded-2xl border px-4 py-3 text-sm text-white outline-none transition ${controlClass}`}
                      >
                        <option value="">Seleccione</option>
                        {(field.options ?? []).map((option) => (
                          <option key={option} value={option}>{option}</option>
                        ))}
                      </select>

                      {hasError && <p className="mt-1 text-xs font-bold text-red-300">Selecciona una opción válida.</p>}
                    </label>
                  );
                }

                return (
                  <label key={field.key}>
                    <span className="mb-2 block text-sm font-bold text-slate-200">{field.label}</span>

                    <input
                      type={field.type ?? "text"}
                      value={form[field.key]}
                      onChange={(event) => updateField(field.key, event.target.value)}
                      placeholder={field.placeholder}
                      className={`w-full rounded-2xl border px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 ${controlClass}`}
                    />

                    {hasError && <p className="mt-1 text-xs font-bold text-red-300">Completa este campo antes de guardar.</p>}
                  </label>
                );
              })}
            </div>

            <div className="mt-6 flex flex-col gap-3 md:flex-row">
              <button type="button" onClick={handleSave} className="rounded-2xl bg-rose-500 px-6 py-3 text-sm font-black text-white shadow-lg shadow-rose-950/50 transition hover:bg-rose-400">
                {editingId ? "Actualizar residuo" : "Guardar residuo"}
              </button>

              <button type="button" onClick={resetForm} className="rounded-2xl border border-slate-600 px-6 py-3 text-sm font-bold text-slate-200 transition hover:bg-slate-800">
                Limpiar formulario
              </button>
            </div>
          </form>

          <section className="rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-xl">
            <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h2 className="text-2xl font-black text-white">Registro maestro de residuos</h2>
                <p className="mt-1 text-sm text-slate-400">Consulta, filtra, edita y exporta registros de residuos.</p>
              </div>

              <button type="button" onClick={exportJson} className="rounded-2xl border border-rose-400/50 px-5 py-3 text-sm font-bold text-rose-200 transition hover:bg-rose-500/10">
                Exportar JSON
              </button>
            </div>

            <div className="mb-5 grid gap-3 md:grid-cols-[1fr_250px]">
              <input
                value={search}
                onChange={(event) => setSearch(event.target.value)}
                className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white outline-none ring-rose-400/40 transition placeholder:text-slate-600 focus:border-rose-400 focus:ring-4"
                placeholder="Buscar por código, área, tipo, lote, estado, QA..."
              />

              <select
                value={filterEstado}
                onChange={(event) => setFilterEstado(event.target.value)}
                className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white outline-none ring-rose-400/40 transition focus:border-rose-400 focus:ring-4"
              >
                <option>Todos</option>
                <option>Generado</option>
                <option>Segregado</option>
                <option>En cuarentena</option>
                <option>Pendiente entrega</option>
                <option>Entregado a gestor</option>
                <option>Dispuesto</option>
                <option>Rechazado</option>
                <option>Requiere investigación</option>
                <option>Requiere CAPA</option>
              </select>
            </div>

            <div className="max-h-[780px] space-y-4 overflow-auto pr-2">
              {filteredRecords.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-950 p-8 text-center text-sm text-slate-400">
                  No hay residuos registrados. Crea el primer registro con datos completos.
                </div>
              ) : (
                filteredRecords.map((record) => (
                  <article key={record.id} className="rounded-2xl border border-slate-700 bg-slate-950 p-5">
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-lg font-black text-white">{record.codigoResiduo} · {record.tipoResiduo}</h3>
                          <StatusPill value={record.decisionQA} />
                          <span className="rounded-full border border-rose-400/30 bg-rose-500/10 px-3 py-1 text-xs font-bold text-rose-200">
                            Riesgo {record.clasificacionRiesgo}
                          </span>
                        </div>

                        <p className="mt-2 text-sm text-slate-400">
                          {record.fecha} · {record.hora} · {record.areaOrigen} · {record.cantidad} {record.unidad}
                        </p>
                      </div>

                      <div className="flex gap-2">
                        <button type="button" onClick={() => handleEdit(record)} className="rounded-xl border border-slate-600 px-3 py-2 text-xs font-bold text-slate-200 hover:bg-slate-800">
                          Editar
                        </button>

                        <button type="button" onClick={() => handleDelete(record.id)} className="rounded-xl border border-red-400/40 px-3 py-2 text-xs font-bold text-red-200 hover:bg-red-500/10">
                          Eliminar
                        </button>
                      </div>
                    </div>

                    <div className="mt-4 grid gap-3 text-sm md:grid-cols-2">
                      <Data label="Empresa" value={record.empresa} />
                      <Data label="Sede" value={record.sede} />
                      <Data label="Ubicación" value={record.ubicacion} />
                      <Data label="Lote relacionado" value={record.codigoLoteRelacionado || "Sin registro"} />
                      <Data label="Contenedor" value={record.contenedor} />
                      <Data label="Generó" value={record.responsableGeneracion} />
                      <Data label="Entregó" value={record.responsableEntrega} />
                      <Data label="Gestor" value={record.gestorAutorizado || "Sin registro"} />
                      <Data label="Tratamiento" value={record.requiereTratamiento} />
                      <Data label="Método disposición" value={record.metodoDisposicion} />
                      <Data label="Fecha disposición" value={record.fechaDisposicion || "Sin registro"} />
                      <Data label="Certificado" value={record.certificadoDisposicion || "Sin registro"} />
                      <Data label="Estado gestión" value={record.estadoGestion} />
                      <Data label="Evidencia" value={record.evidencia || "Sin registro"} />
                    </div>

                    <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-900 p-4 text-sm text-slate-300">
                      <p><span className="font-bold text-slate-100">Descripción: </span>{record.descripcionResiduo}</p>

                      {record.tratamientoAplicado && (
                        <p className="mt-2"><span className="font-bold text-slate-100">Tratamiento: </span>{record.tratamientoAplicado}</p>
                      )}

                      {record.desviacionAsociada && (
                        <p className="mt-2"><span className="font-bold text-slate-100">Desviación: </span>{record.desviacionAsociada}</p>
                      )}

                      {record.accionCorrectiva && (
                        <p className="mt-2"><span className="font-bold text-slate-100">CAPA: </span>{record.accionCorrectiva}</p>
                      )}

                      {record.observaciones && (
                        <p className="mt-2"><span className="font-bold text-slate-100">Observaciones: </span>{record.observaciones}</p>
                      )}
                    </div>

                    <p className="mt-4 text-xs text-slate-500">
                      Creado: {record.creadoEn || "Sin dato"} · Actualizado: {record.actualizadoEn || "Sin dato"}
                    </p>
                  </article>
                ))
              )}
            </div>
          </section>
        </section>
      </section>
    </main>
  );
}

function CloudNotice({
  message,
  errors,
  tone,
  onClose,
}: {
  message: string;
  errors: string[];
  tone: CloudTone;
  onClose: () => void;
}) {
  const isSuccess = tone === "success";

  return (
    <div role="alert" aria-live="assertive" className="fixed right-5 top-5 z-[9999] w-[min(94vw,620px)]">
      <section className={`rounded-[2rem] border-2 bg-slate-950 p-6 text-white shadow-2xl ${isSuccess ? "border-emerald-300" : "border-amber-300"}`}>
        <div className="flex items-start justify-between gap-5">
          <div>
            <p className={`text-xs font-black uppercase tracking-[0.28em] ${isSuccess ? "text-emerald-200" : "text-amber-200"}`}>
              FloraTrack Cloud Notice
            </p>

            <p className="mt-2 text-lg font-black leading-snug text-white md:text-xl">{message}</p>

            {errors.length > 0 && (
              <div className="mt-5 rounded-3xl border border-white/20 bg-white p-5 text-slate-950 shadow-xl">
                <p className="text-sm font-black uppercase tracking-wide text-slate-700">Información pendiente antes de guardar</p>

                <ul className="mt-3 max-h-64 list-disc space-y-2 overflow-auto pl-5 text-sm font-bold leading-relaxed text-slate-950">
                  {errors.map((error) => <li key={error}>{error}</li>)}
                </ul>
              </div>
            )}
          </div>

          <button type="button" onClick={onClose} className="shrink-0 rounded-2xl border border-white/30 bg-white px-4 py-2 text-xs font-black uppercase tracking-wide text-slate-950 shadow-lg transition hover:bg-slate-200">
            Cerrar
          </button>
        </div>
      </section>
    </div>
  );
}

function Metric({ title, value, tone = "slate" }: { title: string; value: number; tone?: "slate" | "emerald" | "amber" | "red" | "sky" }) {
  const toneClass =
    tone === "emerald" ? "text-emerald-300" :
    tone === "amber" ? "text-amber-300" :
    tone === "red" ? "text-red-300" :
    tone === "sky" ? "text-sky-300" :
    "text-white";

  return (
    <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5">
      <p className="text-sm text-slate-400">{title}</p>
      <p className={`mt-2 text-3xl font-black ${toneClass}`}>{value}</p>
    </div>
  );
}

function Data({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 px-4 py-3">
      <p className="text-xs font-bold uppercase tracking-wide text-slate-500">{label}</p>
      <p className="mt-1 font-semibold text-slate-200">{value}</p>
    </div>
  );
}

function StatusPill({ value }: { value: string }) {
  const className =
    value === "Liberado QA" || value === "No aplica"
      ? "border-emerald-400/40 bg-emerald-500/10 text-emerald-200"
      : value === "Pendiente QA"
      ? "border-amber-400/40 bg-amber-500/10 text-amber-200"
      : "border-red-400/40 bg-red-500/10 text-red-200";

  return <span className={`rounded-full border px-3 py-1 text-xs font-bold ${className}`}>{value || "Sin QA"}</span>;
}
